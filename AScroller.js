...
function AScroller(scroller_in, curElement){
  ...
  if(data_ascroller){
    ...
    function curCss( style , scroller_ , prev ){
      ...
      function resize(scroll_i){
        ...
        [scroller,scroller_in,scroller_area].cssScroller({'height' : ''});
          ...
          for(var i = 0; i < scroll_i.children.length; i++){
            ...
            if( m_t > max_m_t ) max_m_t = m_t;
              var m_b = parseInt( window.getComputedStyle(sc_i).marginTop );
            if( m_b > max_m_b ) max_m_b = m_b;
              var height_i = sc_i.offsetHeight;
            if( height_i > height ){
                height = height_i ;
                max_h_el = sc_i;
              }
        ...
        if( width > parentWidth ) width = parentWidth;
          ...
          if( AScrollerBox === scroller_in ){
            AScrollerBoxMax.style.width = window.innerWidth + "px";
            AScrollerBoxMax.style.height = window.innerHeight + "px";
          ...
          if( gallery && width )scroll_i.children.cssScroller({'max-width' : width + "px" 
          ,'width': width + "px" 
          , "max-height" : height + "px" });

          scroll_i.children.cssScroller({ 'max-width' : width + "px" , "max-height" : height + "px" , "height" : "inherit"  });

          
          if( gallery && width ) [scroller,scroller_area].cssScroller({'width': width + ( max_m_t + max_m_b ) + 'px' });
            [scroller_in,scroller_area].cssScroller({'height': height + ( max_m_t + max_m_b ) + 'px' });
      ...
      if(!touches)
        scroller_in.addEventListener("mousedown",  startmove)  
      else
